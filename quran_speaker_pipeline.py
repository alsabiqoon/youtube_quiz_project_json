...
BOUNDARY_GAP_SECONDS = 2.0  # الفاصل الإلزامي بين نهاية السؤال وبداية الإجابة
...
def flatten_words(
    ...
    """
    تحويل جميع الكلمات إلى خط زمني واحد.

    المقطع الواحد قد يحتوي المحكم ورئيس اللجنة والمتسابق معاً،
    لذلك نعتمد على توقيت وهوية المتحدث لكل كلمة.
    """
...
def infer_contestant_speaker_from_words(
    ...
    """
    المتسابق هو صاحب أكبر مدة كلام إجمالية في الحلقة.
    """
...
def is_stop_word(word: str) -> bool:
    """
    مطابقة كاملة فقط، حتى لا تتحول كلمة مثل "يحسب"
    إلى أمر إيقاف بسبب احتوائها على "حسب".
    """
    norm = normalize_arabic(word)
...
def find_question_word_indices(
    ...
    """
    إيجاد بداية كل سؤال من كلمة السؤال نفسها.

    Whisper قد يكرر كلمة السؤال مرتين متتاليتين، لذلك ندمج
    التكرارات التي تقع خلال ثماني ثوانٍ.
    """
    raw_indices = [
        index
        for index, word in enumerate(words)
...
def is_stable_contestant_start(
    words: list[dict[str, Any]],
    ...
    min_words: int = 3,
    min_speech_seconds: float = 1.2,
) -> bool:
    """
    التأكد أن بداية المتسابق ليست كلمة عابرة صُنفت خطأ.

    نعتبر البداية ثابتة عندما نجد خلال ست ثوانٍ:
    - ثلاث كلمات على الأقل للمتسابق، أو
    - مدة كلام تراكمية لا تقل عن 1.2 ثانية.
    """
    candidate_time = float(words[candidate_index]["start"])
    count = 0
    duration = 0.0

    for index in range(candidate_index, stop_index):
        word = words[index]

        if float(word["start"]) - candidate_time > window_seconds:
            break
        ...
        if word["speaker"] == contestant_speaker:
            ...
            if count >= min_words or duration >= min_speech_seconds:
                return True

    return False


def find_stable_contestant_start_index(
    words: list[dict[str, Any]],
    contestant_speaker: str,
    start_index: int,
    stop_index: int,
    ...
    """
    أول بداية مستقرة للمتسابق بعد السؤال.

    هذا يمنع دخول السؤال في الاستعاذة التي يقولها المتسابق،
    ويمنع أيضاً اعتماد كلمة منفردة صُنفت بالخطأ على أنها للمتسابق.
    """
    for index in range(start_index + 1, stop_index):
        if words[index]["speaker"] != contestant_speaker:
            continue

        if is_stable_contestant_start(
            words,
            contestant_speaker,
            index,
            stop_index,
...
def find_question_end_time(
    ...
    """
    نهاية السؤال هي نهاية آخر كلمة لغير المتسابق قبل بداية جوابه.
    """
    question_start = float(words[question_index]["start"])
    answer_start = float(words[answer_index]["start"])
    ...
    return max(
        question_start,
        min(last_end, answer_start - BOUNDARY_GAP_SECONDS),
    )
...
def find_answer_end_time(
    ...
    """
    نهاية الجواب وفق آلة حالات بسيطة:

    ANSWER
      -> آخر حسب/حسبك من غير المتسابق قبل السؤال التالي
      -> أو آخر كلمة للمتسابق إذا لم توجد كلمة إيقاف.
    """
...
def build_questions(
    ...
    """
    آلة الحالات المستخدمة:

    WAIT_QUESTION -> QUESTION -> ANSWER -> WAIT_QUESTION

    الفصل بين السؤال والجواب يعتمد على هوية المتحدث واستقرار
    بداية المتسابق، لا على تشابه النص. لذلك يمكن للمحكم والمتسابق
    أن يقولا الاستعاذة نفسها دون أن تختلط الحدود.
    """
    ...
    for order, question_index in enumerate(question_indices):
        ...
        answer_index = find_stable_contestant_start_index(
            words=words,
        ...
        if answer_index is None:
            print(
                f"تحذير: لم أجد بداية مستقرة للجواب بعد السؤال {order + 1}."
            )
            continue
        ...
        # تحويل الحدود إلى ثوانٍ صحيحة.
        question_start = max(0, math.floor(question_start_value))
        answer_start = max(question_start + 1, math.ceil(answer_start_value))
        answer_end = max(answer_start, math.floor(answer_end_value))

        # قاعدة نهائية صارمة:
        # نهاية السؤال = قبل بداية الإجابة بثانيتين دائماً.
        # لا نعتمد هنا على نهاية كلام المحكم التي قد تكون ناقصة بسبب WhisperX.
        gap_seconds = max(1, int(round(BOUNDARY_GAP_SECONDS)))
        question_end = answer_start - gap_seconds

        # حماية للحالات النادرة التي تكون فيها بداية الإجابة قريبة جداً.
        if question_end <= question_start:
            question_end = question_start + 1
            answer_start = max(answer_start, question_end + gap_seconds)
        ...
        # فحص نهائي يمنع تكرار مشكلة السؤال الرابع.
        actual_gap = (
            question_data["answerStart"]
            - question_data["questionEnd"]
        )
        if actual_gap != gap_seconds:
            question_data["questionEnd"] = (
                question_data["answerStart"] - gap_seconds
            )

        print(
            f'السؤال {question_data["id"]}: '
            f'{question_data["questionStart"]} -> '
            f'{question_data["questionEnd"]} | '
            f'الإجابة {question_data["answerStart"]} -> '
            f'{question_data["answerEnd"]} | '
            f'الفاصل={question_data["answerStart"] - question_data["questionEnd"]} ثانية'
        )
...
def safe_slug(value: str) -> str:
    """
    إنشاء اسم آمن للمجلدات والمعرّفات.
    يدعم العربية والإنجليزية، ويستبدل الرموز غير المناسبة بشرطة سفلية.
    """
    value = (value or "").strip()
    value = re.sub(r"[^\w\u0600-\u06FF-]+", "_", value, flags=re.UNICODE)
    value = re.sub(r"_+", "_", value).strip("_")
    return value or "competition"



def arabic_episode_title(number: int) -> str:
    names = {
    ...
    return f"الحلقة {names[number]}" if number in names else f"الحلقة {number}"


def get_episode_number(competition_dir: Path, episode_id: str, video_id: str) -> int:
    index_path = competition_dir / "episodes.json"
    ...
    for index, item in enumerate(episodes):
        if isinstance(item, dict) and (item.get("id") == episode_id or item.get("videoId") == video_id):
            return index + 1
    return len(episodes) + 1

def update_episodes_index(
    ...
    contestant: str,
    video_id: str,
    json_file_relative: str,
) -> Path:
    """
    إنشاء أو تحديث ملف episodes.json داخل مجلد المسابقة.

    إذا كانت الحلقة موجودة مسبقاً، يتم تحديث بياناتها بدلاً من تكرارها.
    """
    index_path = competition_dir / "episodes.json"
    ...
    for index, item in enumerate(episodes):
        if not isinstance(item, dict):
            continue
        if item.get("id") == episode_id or item.get("videoId") == video_id:
            episodes[index] = episode_record
            replaced = True
            break
...
def process_video(
    ...
    competition_id = safe_slug(competition_id)
    competition_dir = output_dir / competition_id
    competition_dir.mkdir(parents=True, exist_ok=True)

    print("\n" + "=" * 70)
    print(f"معالجة الفيديو: {video_id}")

    output_path = competition_dir / f"{video_id}.json"
    debug_path = competition_dir / f"{video_id}_speakers.json"

    # إذا كان ملف الحلقة النهائي موجوداً، نتخطى المعالجة كاملة.
    # هذا يسمح بإيقاف البرنامج وإعادة تشغيله لاحقاً ليكمل من حيث توقف.
    if output_path.exists():
        print(f"✓ تم تخطي الفيديو لأنه موجود مسبقاً: {output_path}")
        return output_path
    ...
    episode_id = f"{competition_id}_{video_id}"
    ...
    resolved_episode_title = (
        ...
        else arabic_episode_title(episode_number)
    )
    ...
    json_file_relative = (
        f"assets/youtube_competitions/{competition_id}/{video_id}.json"
    )
    episodes_index_path = update_episodes_index(
        competition_dir=competition_dir,
        competition_id=competition_id,
        competition_name=competition_name,
        episode_id=episode_id,
        episode_title=resolved_episode_title,
        contestant="",
        video_id=video_id,
        json_file_relative=json_file_relative,
    )

    print(f"العنوان: {info.get('title', '')}")
    print(f"عدد الأسئلة: {len(questions)}")
    print(f"تم إنشاء: {output_path}")
    print(f"تم تحديث: {episodes_index_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--competition-id",
        required=True,
        help="معرف المسابقة، مثل: dubai أو makkah أو iraq",
    )
    parser.add_argument(
        "--competition-name",
        required=True,
        help="اسم المسابقة الظاهر داخل التطبيق",
    )
    parser.add_argument(
        "--episode-title",
        default="",
        help="عنوان الحلقة. إذا تركته فارغاً يُستخدم عنوان فيديو YouTube.",
    )
    parser.add_argument(
        "--contestant",
        default="",
        help="مهمل في هذه النسخة؛ التطبيق يعرض رقم الحلقة فقط.",
    )
    parser.add_argument(
        "--output-dir",
        default="assets/youtube_competitions",
        help="مجلد إخراج المسابقات",
    )
...
def main() -> None:
    ...
    if failures:
        failure_dir = output_dir / safe_slug(args.competition_id)
        failure_dir.mkdir(parents=True, exist_ok=True)
        failure_path = failure_dir / "failures.json"
    ...
    print(f"تمت معالجة ملفات جديدة: {processed_count}")
    print(f"تم تخطي ملفات موجودة: {skipped_count}")
    print(f"عدد الملفات الفاشلة: {len(failures)}")
