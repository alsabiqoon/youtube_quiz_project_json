...
def build_questions(
    ...
    for order, question_index in enumerate(question_indices):
        ...
        # floor لنهاية السؤال وceil لبداية الجواب يمنعان التداخل
        # بعد تحويل الأزمنة إلى أعداد صحيحة.
        question_start = math.floor(question_start_value)
        question_end = math.floor(question_end_value)
        answer_start = math.ceil(answer_start_value)
        answer_end = math.floor(answer_end_value)

        # النهاية النهائية للسؤال تكون دائماً قبل بداية الجواب
        # بثانيتين كاملتين، مهما كانت مدة الصمت أو التداخل.
        question_end = max(
            question_start,
            answer_start - int(BOUNDARY_GAP_SECONDS),
        )
